...
export function resolveMatch(results, anime, lang = 'sub') {
  ...
  const aniListSeason = extractSeason(anime.title?.romaji || anime.title?.english || "");
  const aniListPart = extractPart(anime.title?.romaji || anime.title?.english || "");

  const titles = [];
  // Romaji first for better Anikai matching accuracy
